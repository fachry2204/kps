...
async function deployForce() {
  ...
  try {
    console.log("--- STRATEGIC CLOUD DEPLOYMENT INITIATED ---");
    console.log("Connecting to kps.garudaserver.cloud...");
    ...
    console.log("Cleared existing cloud assignments and reset personnel status.");
    ...
    console.log(`Starting deployment for ${allOps.length} missions using ${allPersonnel.length} personnel on Cloud...`);
    ...
    for (const op of allOps) {
      ...
      console.log(`[CLOUD DEPLOYED] ${op.name}: 1 Commander, ${assignedCount} Members.`);
    }

    console.log("\n--- CLOUD MISSION SUCCESS: ALL FORCES DEPLOYED ---");
    process.exit(0);
  } catch (error) {
    console.error("CLOUD DEPLOYMENT FAILURE:", error);
    process.exit(1);
  }
}
