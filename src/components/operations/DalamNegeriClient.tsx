...
import { Shield, MapPin, Users, Calendar, ArrowRight, Target, Activity } from "lucide-react";
import { useState } from "react";

import { SATGAS_DALAM_NEGERI as SATGAS_LIST } from "@/lib/constants";

export default function DalamNegeriClient({ initialOperations }: { initialOperations: any[] }) {
  const [searchQuery, setSearchQuery] = useState("");

  const filteredSatgas = SATGAS_LIST.filter(s => 
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-tactical-border pb-6">
        <div>
          <h2 className="text-2xl font-bold text-tactical-text tracking-tighter flex items-center gap-3">
            <Shield className="text-tactical-green" /> GELAR OPERASI DALAM NEGERI
          </h2>
          <p className="text-tactical-muted font-mono text-xs mt-1 uppercase tracking-widest">
            Domestic Task Force Deployment & Operational Status
          </p>
        </div>
        ...
        <div className="relative">
          ...
          <input 
            type="text"
            placeholder="Search Satgas / Location..."
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
            className="bg-tactical-panel border border-tactical-border rounded-md pl-10 pr-4 py-2 text-sm font-mono text-tactical-text focus:outline-none focus:border-tactical-green w-full md:w-80 transition-all"
          />
        </div>
      </div>
          ...
          >
            ...
            <div className="relative z-10">
              <div className="flex justify-between items-start mb-4">
                <div>
                  <h3 className="text-lg font-bold text-tactical-text group-hover:text-tactical-green transition-colors">{satgas.name}</h3>
              ...
              <div className="mb-6">
                <div className="p-2 bg-tactical-bg/50 border border-tactical-border rounded">
                  <div className="text-[9px] font-mono text-tactical-muted mb-1">PERSONIL BERTUGAS</div>
              ...
              <div className="pt-2 border-t border-tactical-border/30">
                  <button 
                    onClick={() => window.location.href = `/gelar-operasi/dalam-negeri/detail/${satgas.id}`}
